...
import { CheckCircle } from "lucide-react" // CheckCircle icon-ийг импортлов
...
interface SurveyCardProps {
  ...
  isCompleted?: boolean // Судалгаа бөглөгдсөн эсэхийг илтгэх проп
  creatorName: string // New prop
  creatorImageSrc: string // New prop
}
...
export function SurveyCard({
  ...
  creatorImageSrc, // Destructure new prop
}: SurveyCardProps) {
  ...
  return (
    <Link href={`/surveys/${id}`} className="block">
      <Card className="relative w-full aspect-square overflow-hidden rounded-lg bg-zinc-100 text-zinc-900 shadow-md hover:shadow-lg transition-shadow cursor-pointer dark:bg-zinc-800 dark:text-white">
        ...
        {isCompleted && ( // isCompleted проп үнэн бол энэ хэсэг харагдана
          <div className="absolute top-2 right-2 bg-green-600 text-white text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1 z-10">
            <CheckCircle className="h-3 w-3" /> {/* Зөв icon */}
          </div>
        )}

        {/* Creator Profile Info - Added to the top left corner */}
        <div className="absolute top-2 left-2 flex items-center gap-1 bg-black/50 text-white text-xs font-bold px-2 py-1 rounded-full z-10">
          <Image
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-4">
          ...
          <CardHeader className="p-0 pt-8">
            {" "}
            {/* Added pt-8 here to push title down */}
